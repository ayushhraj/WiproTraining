...
# Child - select all the td element that are direct children of a row
'''
...
print(f"Parent Example -> Email '{email_cell.text}' belongs to row wiitrh first name : "
...
#Descendant -find all descendant (cells) under table body
'''
...
#above -> elements located above another
...
elmt_below_username = driver.find_element(
    locate_with(By.TAG_NAME, "input").below(username_field)
)
print(f"below Example -> Placeholder below username: {elmt_below_username.get_attribute('placeholder')}")
elmt_below_username.send_keys('secret_sauce')
time.sleep(2)
login_button.click()
time.sleep(3)

#torightof -> elements to the left of another
...
print(f"toRightOf Example -> Elements to the right of Twitter icon has href: {facebook_icon.get_attribute('href')}")

#toleftof -> elements to the right of another
...
for element in near_twitter:
    print(f"Near Twitter -> Elements near Facebook icon has href: {element.get_attribute('href')}")
